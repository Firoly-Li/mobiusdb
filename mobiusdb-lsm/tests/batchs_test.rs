...
use arrow::{
    ...
    compute::{sort_to_indices, SortOptions},
    datatypes::*,
};
...
use mobiusdb_lsm::{
    memtable::array_data_utils::{merge_batches, merge_batches_with_schema, merge_schema},
...
fn batch_sort_test() {
    ...
    let sorted_indices =
        sort_to_indices(&batch.column(sort_column_index), Some(sort_options), None).unwrap();
    ...
    let sorted_batch = RecordBatch::try_new(schema, vec![sorted_ids, sorted_names]).unwrap();
