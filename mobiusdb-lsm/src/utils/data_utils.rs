...
pub async fn get_timestamp_from_batch(batch: &RecordBatch) -> Result<(u64, u64)> {
    ...
    let fd = df.select_columns(&["timestamp"])?.collect().await?;
...
pub fn batch_sort(batch: &RecordBatch, sort_by: &str) -> Result<RecordBatch> {
    ...
    let sorted_indices =
        sort_to_indices(&batch.column(sort_column_index), Some(sort_options), None)?;
    ...
    let sorted_batch = RecordBatch::try_new(schema, vs)?;
    Ok(sorted_batch)
}
